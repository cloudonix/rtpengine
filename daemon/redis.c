...
static int redis_update_call_streams(struct call *c, struct redis_list *streams) {
	...
	for (i = 0, pk = c->streams.head; pk && (i < streams->len); pk = pk->next, i++) {
		...
		ZERO(endpoint);
		if (redis_hash_get_unsigned((unsigned int *) &ps_flags, rh, "ps_flags"))
			return -1;
		if (redis_hash_get_endpoint(&endpoint, rh, "endpoint"))
			return -1;
		if (redis_hash_get_endpoint(&advertised_endpoint, rh, "advertised_endpoint"))
			return -1;
...
static int redis_update_call_tags(struct call *c, struct redis_hash *call, JsonReader* root_reader) {
	struct redis_list tags;
	unsigned int i;
//	int ii;
	struct redis_hash *rh;
//	struct call_monologue *ml;
...
// 	// this is how json_link_tags looks
// 	unsigned int i;
// 	struct call_monologue *ml, *other_ml;
// 	GQueue q = G_QUEUE_INIT;
// 	GList *l;
...
static str* json_reader_get_str(JsonReader *reader, const char *key) {
	const gchar *strval;
	str *out = NULL;
...
/*
static str* json_reader_get_str_element(JsonReader *reader, unsigned idx) {
...
static long long json_reader_get_ll_element(JsonReader *reader, unsigned idx) {
	const gchar *strval;
	JsonNode *node;
	long long out = -1;
	if (json_reader_read_element(reader, idx)) {
		...
		if (json_node_get_value_type(node) == G_TYPE_STRING) {
			strval = json_node_get_string(node);
			if (strval)
				out = strtoll(strval, NULL, 10);
		} else {
...
static redis_call_media_stream_fd_t *redis_call_media_stream_fd_create(unsigned unique_id, JsonNode *json) {
	...
	if ((llval = json_reader_get_ll(reader, "stream")) >= 0)
		streamfdref->stream_unique_id = llval;
	else
		goto fail;
	streamfdref->pref_family = json_reader_get_str(reader, "pref_family");
	if ((llval = json_reader_get_ll(reader, "localport")) >= 0)
		streamfdref->localport = llval;
	streamfdref->logical_intf = json_reader_get_str(reader, "logical_intf");
	if ((llval = json_reader_get_ll(reader, "local_intf_uid")) >= 0)
		streamfdref->logical_intf_uid = llval;
...
static redis_call_media_stream_t *redis_call_media_stream_create(unsigned unique_id, JsonNode *json, GQueue *sfds) {
	...
	if ((llval = json_reader_get_ll(reader, "media")) >= 0)
		streamref->media_unique_id = llval;
	else
		goto fail;
	if ((llval = json_reader_get_ll(reader, "sfd")) >= 0)
		streamref->selected_sfd = llval;
	if ((llval = json_reader_get_ll(reader, "rtp_sink")) >= 0)
		streamref->rtp_sink = llval < 1000 ? llval : -1; /* old code writes -1 as u_int32_t */
	if ((llval = json_reader_get_ll(reader, "rtcp_sink")) >= 0)
		streamref->rtcp_sink = llval < 1000 ? llval : -1; /* old code writes -1 as u_int32_t */
	if ((llval = json_reader_get_ll(reader, "rtcp_sibling")) >= 0)
		streamref->rtcp_sibling = llval < 1000 ? llval : -1; /* old code writes -1 as u_int32_t */
...
static redis_call_t* redis_call_create(const str* callid, JsonNode* json) {
	redis_call_t* callref = NULL;
